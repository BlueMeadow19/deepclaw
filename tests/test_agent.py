...
class TestCompactionHelpersRemoved:
    def test_custom_compaction_normalization_helpers_are_removed(self):
        ...
        assert not hasattr(agent_mod, "_build_compaction_fallback_summary")
        assert not hasattr(agent_mod, "_summary_loses_active_task")
        assert not hasattr(agent_mod, "_contains_generic_placeholder")

    def test_custom_summarization_factory_helpers_are_removed(self):
        assert not hasattr(agent_mod, "_create_deepclaw_summarization_middleware")
        assert not hasattr(agent_mod, "_create_deepclaw_summarization_tool_middleware")
        assert not hasattr(agent_mod, "_patched_deepagents_summarization_factory")

    def test_builds_production_subagents_without_manual_compaction(self):
